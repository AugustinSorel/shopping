...
pub fn sign_up_post(body: auth.SignUpInput, handle_response) {
  let body =
    json.object([
      #("email", json.string(body.email)),
      #("password", json.string(body.password)),
...
pub fn sign_out_post(handle_response) {
  rsvp.post(
    "/sign-out",
    json.object([]),
    rsvp.expect_ok_response(handle_response),
  )
}

pub fn decode_sign_up_form(values: List(#(String, String))) {
  form.decoding({
...
pub fn decode_sign_in_form(values: List(#(String, String))) {
  form.decoding({
    use email <- form.parameter
    use password <- form.parameter
