...
function must_be_string_longer_than(length4) {
  return (input3) => {
    let $ = string_length(input3) > length4;
    if ($) {
      return new Ok(input3);
    } else {
      return new Error(
        "Must be longer than " + to_string(length4) + " characters"
      );
    }
  };
}
function must_be_string_shorter_than(length4) {
  return (input3) => {
    let $ = string_length(input3) < length4;
    if ($) {
      return new Ok(input3);
      ...
      return new Error(
        "Must be shorter than " + to_string(length4) + " characters"
      );
    }
  };
}
...
function set_header2(request, key, value3) {
...
// build/dev/javascript/shared/shared/auth.mjs
var SignUpInput = class extends CustomType {
...
var Destructive = class extends CustomType {
};
...
function alert(variant, attr, children) {
  let base_class = "relative w-full p-4 [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg+div]:translate-y-[-3px] [&:has(svg)]:pl-11 rounded-lg border";
  let _block;
  if (variant instanceof Default) {
    _block = "border-border text-foreground";
  } else if (variant instanceof Ghost) {
    _block = "border-border text-foreground";
  } else {
    _block = "border-error text-on-error-container [&>svg]:text-on-error-container bg-error-container";
  }
  let variant_class = _block;
  let attr_class = extract_class(attr);
  let class$2 = doTwMerge(
    toList([base_class, variant_class, attr_class])
  );
  return div(
    prepend(
      role("alert"),
      prepend(class$(class$2), attr)
    ),
    children
  );
}
function alert_title(attr, children) {
  return h5(
    prepend(
...
function alert_description(attr, children) {
  return div(
    prepend(class$("[&_p]:leading-relaxed text-sm"), attr),
    children
  );
}
...
function spinner2(attr, size2) {
  return spinner(
    prepend(class$("animate-spin"), attr),
    size2
  );
}
...
function sign_up(body, handle_response) {
  let body$1 = object2(
    toList([
      ["email", string4(body.email)],
      ["password", string4(body.password)],
      ["confirm_password", string4(body.confirm_password)]
    ])
  );
  return post(
    "/sign-up",
      ...
      (email) => {
        return parameter(
          (password) => {
            return parameter(
              (confirm_password) => {
                return new SignUpInput(email, password, confirm_password);
              }
            );
          }
        );
...
function view3(form2, state, on_submit2) {
  return main(
    toList([class$("max-w-app mx-auto py-10 space-y-15")]),
    toList([
      h1(
...
var ApiReturnedSignUp = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function init2(_) {
  ...
  if ($ instanceof Ok) {
    ...
    _block = new SignUp(new$(), new Idle());
  }
  let route = _block;
      ...
      if ($1 instanceof Ok) {
        ...
        return [
          (() => {
            ...
            return new Model(
              (() => {
                let _record$1 = sign_up2;
                return new SignUp(_record$1.form, new Loading());
              })(),
              _record.user
            );
          })(),
          sign_up(
            form$1,
            (var0) => {
              return new ApiReturnedSignUp(var0);
            }
          )
        ];
      } else {
      ...
      return [
        (() => {
          ...
          return new Model(
            (() => {
              let _record$1 = sign_up2;
              return new SignUp(_record$1.form, new Err(msg$1));
            })(),
            _record.user
          );
        })(),
